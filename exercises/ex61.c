...
"-variable   : use variable material coefficients\n"
"\n";
...
typedef struct {
  ...
  DM        dm_bouyancy,dm_lame;
  Vec       bouyancy,lame; /* Global, but for efficiency one could store local vectors */
  PetscInt  dim; /* 2 or 3 */
  PetscReal rho,lambda,mu; /* constant material properties */
  PetscBool variable;      /* Use variable material properties */
...
static PetscErrorCode CreateLameVariable(Ctx*);
...
int main(int argc,char *argv[])
{
  ...
  ierr = PetscOptionsGetBool(NULL,NULL,"-variable",&ctx.variable,NULL);CHKERRQ(ierr);
  ...
  /* Create a DMStag, with uniform coordinates, for the velocities */
  {
    ...
    switch (ctx.dim) {
      case 2:
        ...
        ierr = DMStagCreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_PERIODIC,DM_BOUNDARY_PERIODIC,100,100,PETSC_DECIDE,PETSC_DECIDE,dof0,dof1,dof2,DMSTAG_STENCIL_BOX,stencilWidth,NULL,NULL,&ctx.dm_velocity);CHKERRQ(ierr);
        break;
      case 3:
  ...
  /* Create two additional DMStag objects for the bouyancy and Lame parameters */
  switch (ctx.dim) {
    case 2:
      /* Bouyancy on both element boundaries */
      ierr = DMStagCreateCompatibleDMStag(ctx.dm_velocity,0,1,0,0,&ctx.dm_bouyancy);CHKERRQ(ierr);
      break;
    case 3:
      ierr = DMStagCreateCompatibleDMStag(ctx.dm_velocity,0,0,1,0,&ctx.dm_bouyancy);CHKERRQ(ierr);
      break;
    default: SETERRQ1(PETSC_COMM_WORLD,PETSC_ERR_SUP,"Not Implemented for dimension %D",ctx.dim);
  }
  ierr = DMSetUp(ctx.dm_bouyancy);CHKERRQ(ierr);
  ...
  ierr = DMStagSetUniformCoordinatesProduct(ctx.dm_lame,ctx.xmin,ctx.xmax,ctx.ymin,ctx.ymax,ctx.zmin,ctx.zmax);CHKERRQ(ierr);
  ...
  /* Print out some info */
  {
    ...
    ierr = DMStagGetGlobalSizes(ctx.dm_velocity,&Nx,NULL,NULL);CHKERRQ(ierr);
    dx = (ctx.xmax - ctx.xmin)/Nx;
  ...
  if (ctx.variable) {
    ierr = CreateLameVariable(&ctx);CHKERRQ(ierr);
  } else {
    ierr = CreateLame(&ctx);CHKERRQ(ierr);
  }
  ierr = DMCreateGlobalVector(ctx.dm_bouyancy,&ctx.bouyancy);CHKERRQ(ierr);
  ierr = VecSet(ctx.bouyancy,1.0/ctx.rho);CHKERRQ(ierr);
  ...
  for (timestep = 1; timestep <= ctx.timesteps; ++timestep) {
    ...
    ierr = UpdateVelocity(&ctx,velocity,stress,ctx.bouyancy);CHKERRQ(ierr);
  ...
  ierr = VecDestroy(&ctx.bouyancy);CHKERRQ(ierr);
  ierr = DMDestroy(&ctx.dm_velocity);CHKERRQ(ierr);
  ierr = DMDestroy(&ctx.dm_stress);CHKERRQ(ierr);
  ierr = DMDestroy(&ctx.dm_bouyancy);CHKERRQ(ierr);
...
{
  PetscErrorCode ierr;
  PetscInt       ex,ey,ez,startx,starty,startz,nx,ny,nz,extrax,extray,extraz;

  PetscFunctionBeginUser;
  ierr = DMCreateGlobalVector(ctx->dm_lame,&ctx->lame);CHKERRQ(ierr);
  ierr = DMStagGetCorners(ctx->dm_bouyancy,&startx,&starty,&startz,&nx,&ny,&nz,&extrax,&extray,&extraz);CHKERRQ(ierr);
  if (ctx->dim == 2) {
    /* Element values */
    ...
    for (ez=startz; ez<startz+nz; ++ez) {
      for (ey=starty; ey<starty+ny+extray; ++ey) {
        for (ex=startx; ex<startx+nx+extrax; ++ex) {
          DMStagStencil   pos[3];
          const PetscReal val[3] = {ctx->mu,ctx->mu,ctx->mu};

          pos[0].i = ex; pos[0].j = ey; pos[0].k = ez; pos[0].c = 0; pos[0].loc = DMSTAG_DOWN_LEFT;
          pos[1].i = ex; pos[1].j = ey; pos[1].k = ez; pos[1].c = 0; pos[1].loc = DMSTAG_BACK_DOWN;
          pos[2].i = ex; pos[2].j = ey; pos[2].k = ez; pos[2].c = 0; pos[2].loc = DMSTAG_BACK_LEFT;
          ierr = DMStagVecSetValuesStencil(ctx->dm_lame,ctx->lame,3,pos,val,INSERT_VALUES);CHKERRQ(ierr);
        }
      }
    }
...
static PetscReal LambdaFunction(PetscReal x,PetscReal y,PetscReal z) {(void)y; (void)z; return x < 0.6 ? 1.0: 0.1;}
static PetscReal     MuFunction(PetscReal x,PetscReal y,PetscReal z) {(void)z; (void)x; return y < 0.6 ? 1.0: 0.1;}

/* Variable-coefficient example, using direct array access for local vectors */
static PetscErrorCode CreateLameVariable(Ctx *ctx)
{
  PetscErrorCode ierr;
  PetscInt       ex,ey,startx,starty,startz,nx,ny,nz,extrax,extray,extraz;
  PetscReal      **arr_coordinates_x,**arr_coordinates_y,**arr_coordinates_z;
  PetscInt       slot_lambda_element,slot_mu_element,slot_mu_vertex;
  ...
  ierr = DMStagGetProductCoordinateArraysRead(ctx->dm_lame,&arr_coordinates_x,&arr_coordinates_y,&arr_coordinates_z);CHKERRQ(ierr);
  ierr = DMStagGetProductCoordinateLocationSlot(ctx->dm_lame,DMSTAG_ELEMENT,&slot_center);CHKERRQ(ierr);
  ierr = DMStagGetProductCoordinateLocationSlot(ctx->dm_lame,DMSTAG_LEFT,&slot_prev);CHKERRQ(ierr);
  ...
  /* Get temporary access to local vector and array */
  ierr = DMGetLocalVector(ctx->dm_lame,&lame_local);CHKERRQ(ierr);

  if (ctx->dim == 2) {
    PetscScalar ***arr_lame;

    /* An additional slot number, dimension dependent */
    ierr = DMStagGetLocationSlot(ctx->dm_lame,DMSTAG_DOWN_LEFT,0,&slot_mu_vertex);CHKERRQ(ierr);

    /* Get access to raw array */
    ierr = DMStagVecGetArray(ctx->dm_lame,lame_local,&arr_lame);CHKERRQ(ierr);
    ...
    for (ey=starty; ey<starty+ny; ++ey) {
      for (ex=startx; ex<startx+nx; ++ex) {
        ...
        arr_lame[ey][ex][slot_lambda_element] = LambdaFunction(x,y,0.0);
        arr_lame[ey][ex][slot_mu_element]     = MuFunction(x,y,0.0);
      }
    }
    /* Vertex Values */
    for (ey=starty; ey<starty+ny+extray; ++ey) {
      for (ex=startx; ex<startx+nx+extrax; ++ex) {
        const PetscReal x = arr_coordinates_x[ex][slot_prev];
        const PetscReal y = arr_coordinates_y[ey][slot_prev];

        arr_lame[ey][ex][slot_mu_vertex] = MuFunction(x,y,0.0);
      }
    }
    ierr = DMStagVecRestoreArray(ctx->dm_lame,lame_local,&arr_lame);CHKERRQ(ierr);
  } else SETERRQ1(PetscObjectComm((PetscObject)ctx->dm_velocity),PETSC_ERR_SUP,"Unsupported dim %d",ctx->dim);

  /* Map to global vector and release local vector */
  ierr = DMLocalToGlobal(ctx->dm_lame,lame_local,INSERT_VALUES,ctx->lame);CHKERRQ(ierr);
...
static PetscErrorCode ForceStress(const Ctx *ctx,Vec stress, PetscReal t)
{
  ...
  DMStagStencil     pos;
  PetscBool         this_rank;
  const PetscScalar val = PetscExpReal(-10.0 * t);
  ...
  this_rank = start[0] <= N[0]/2 && N[0]/2 <= start[0] + n[0];
  this_rank = this_rank && start[1] <= N[1]/2 && N[1]/2 <= start[1] + n[1];
  if(ctx->dim == 3 ) this_rank = this_rank && start[2] <= N[2]/2 && N[2]/2 <= start[2] + n[2];
  if (this_rank) {
  ...
  /* Will scatter to the appropriate rank */
  ierr = VecAssemblyBegin(stress);CHKERRQ(ierr);
  ierr = VecAssemblyEnd(stress);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

static PetscErrorCode UpdateVelocity_2d(const Ctx *ctx,Vec velocity,Vec stress, Vec bouyancy)
{
  PetscErrorCode    ierr;
  Vec               velocity_local,stress_local,bouyancy_local;
  PetscInt          ex,ey,startx,starty,nx,ny;
  PetscInt          slot_coord_next,slot_coord_element,slot_coord_prev;
  PetscInt          slot_vx_left,slot_vy_down,slot_bouyancy_down,slot_bouyancy_left;
  PetscInt          slot_txx,slot_tyy,slot_txy_downleft,slot_txy_downright,slot_txy_upleft;
  const PetscScalar **arr_coord_x,**arr_coord_y;
  const PetscScalar ***arr_stress,***arr_bouyancy;
  PetscScalar       ***arr_velocity;

  PetscFunctionBeginUser;

  /* Prepare direct access to bouyancy data */
  ierr = DMStagGetLocationSlot(ctx->dm_bouyancy,DMSTAG_LEFT,0,&slot_bouyancy_left);CHKERRQ(ierr);
  ierr = DMStagGetLocationSlot(ctx->dm_bouyancy,DMSTAG_DOWN,0,&slot_bouyancy_down);CHKERRQ(ierr);
  ierr = DMGetLocalVector(ctx->dm_bouyancy,&bouyancy_local);CHKERRQ(ierr);
  ierr = DMGlobalToLocal(ctx->dm_bouyancy,bouyancy,INSERT_VALUES,bouyancy_local);CHKERRQ(ierr);
  ierr = DMStagVecGetArrayRead(ctx->dm_bouyancy,bouyancy_local,&arr_bouyancy);CHKERRQ(ierr);
  ...
  for (ey = starty; ey < starty + ny; ++ey) {
    for (ex = startx; ex < startx + nx; ++ex) {
      ...
      if (ey > 0) {
        ...
        const PetscScalar B = arr_bouyancy[ey][ex][slot_bouyancy_down];
      ...
      if (ex > 0) {
        ...
        const PetscScalar B = arr_bouyancy[ey][ex][slot_bouyancy_left];
  ...
  /* Restore all access */
  ierr = DMStagVecRestoreArrayRead(ctx->dm_bouyancy,bouyancy_local,&arr_bouyancy);CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ctx->dm_bouyancy,&bouyancy_local);CHKERRQ(ierr);
...
static PetscErrorCode UpdateVelocity_3d(const Ctx *ctx,Vec velocity,Vec stress, Vec bouyancy)
{
  PetscErrorCode    ierr;
  Vec               velocity_local,stress_local,bouyancy_local;
  PetscInt          ex,ey,ez,startx,starty,startz,nx,ny,nz;
  PetscInt          slot_coord_next,slot_coord_element,slot_coord_prev;
  PetscInt          slot_vx_left,slot_vy_down,slot_vz_back,slot_bouyancy_down,slot_bouyancy_left,slot_bouyancy_back;
  ...
  ierr = DMStagGetLocationSlot(ctx->dm_bouyancy,DMSTAG_LEFT,0,&slot_bouyancy_left);CHKERRQ(ierr);
  ierr = DMStagGetLocationSlot(ctx->dm_bouyancy,DMSTAG_DOWN,0,&slot_bouyancy_down);CHKERRQ(ierr);
  ierr = DMStagGetLocationSlot(ctx->dm_bouyancy,DMSTAG_BACK,0,&slot_bouyancy_back);CHKERRQ(ierr);
  ierr = DMGetLocalVector(ctx->dm_bouyancy,&bouyancy_local);CHKERRQ(ierr);
  ierr = DMGlobalToLocal(ctx->dm_bouyancy,bouyancy,INSERT_VALUES,bouyancy_local);CHKERRQ(ierr);
  ierr = DMStagVecGetArrayRead(ctx->dm_bouyancy,bouyancy_local,&arr_bouyancy);CHKERRQ(ierr);
    ...
    for (ey = starty; ey < starty + ny; ++ey) {
      for (ex = startx; ex < startx + nx; ++ex) {
        ...
        if (ey > 0) {
          ...
          const PetscScalar B = arr_bouyancy[ez][ey][ex][slot_bouyancy_down];
        ...
        if (ex > 0) {
          ...
          const PetscScalar dy = arr_coord_y[ey][slot_coord_next]    - arr_coord_y[ey  ][slot_coord_prev];
          const PetscScalar dz = arr_coord_z[ez][slot_coord_next]    - arr_coord_z[ez  ][slot_coord_prev];
          const PetscScalar B = arr_bouyancy[ez][ey][ex][slot_bouyancy_left];
          ...
          arr_velocity[ez][ey][ex][slot_vx_left] += B * ctx->dt * (
              ...
              + (arr_stress[ez][ey][ex][slot_txz_frontleft] - arr_stress[ez][ey][ex  ][slot_txz_frontleft]) / dz );
        }
        ...
        if (ez > 0) {
          ...
          const PetscScalar B = arr_bouyancy[ez][ey][ex][slot_bouyancy_back];
...
static PetscErrorCode UpdateVelocity(const Ctx *ctx,Vec velocity,Vec stress, Vec bouyancy)
{
  PetscErrorCode ierr;

  PetscFunctionBeginUser;
  if (ctx->dim == 2) {
    ierr = UpdateVelocity_2d(ctx,velocity,stress,bouyancy);CHKERRQ(ierr);
  } else if (ctx->dim == 3) {
    ierr = UpdateVelocity_3d(ctx,velocity,stress,bouyancy);CHKERRQ(ierr);
  } else SETERRQ1(PetscObjectComm((PetscObject)ctx->dm_velocity),PETSC_ERR_SUP,"Unsupported dim %d",ctx->dim);
  PetscFunctionReturn(0);
}
      ...
      for (ex = startx; ex < startx + nx; ++ex) {
        ...
        /* Update tau_xx, tau_yy, and tau_zz*/
        {
          ...
          arr_stress[ez][ey][ex][slot_tzz] +=
              Lp2M * ctx->dt * (arr_velocity[ez][ey][ex][slot_vz_front] - arr_velocity[ez][ey][ex][slot_vz_back]) / dy
            + L    * ctx->dt * (arr_velocity[ez][ey][ex][slot_vx_right] - arr_velocity[ez][ey][ex][slot_vx_left]) / dx
            + L    * ctx->dt * (arr_velocity[ez][ey][ex][slot_vy_up]    - arr_velocity[ez][ey][ex][slot_vy_down]) / dz;
        }
...
/*TEST
   ...
   test:
      ...
      requires: !complex
      nsize: 1
      args: -stag_grid_x 12 -stag_grid_y 7 -nsteps 4

   test:
      suffix: 2
      requires: !complex
      nsize: 9
      args: -stag_grid_x 12 -stag_grid_y 15 -nsteps 3

   test:
      suffix: 3
      requires: !complex
      nsize: 1
      args: -stag_grid_x 12 -stag_grid_y 7 -stag_grid_z 11 -nsteps 2 -dim 3

   test:
      suffix: 4
      requires: !complex
      nsize: 12
      args: -stag_grid_x 12 -stag_grid_y 15 -stag_grid_z 8 -nsteps 3 -dim 3
